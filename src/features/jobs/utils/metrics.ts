import { JobFilters, JobItem, AnalyticsResult, MonthlyPoint, SkillsSeriesPoint, TopSkill, EmergingSkillTrend, EmergingSkillTrendPayload } from '../types/types';
...
export function computeEmergingSkillsTrends(
  ...
  if (monthsWindow > 0 && months.length > monthsWindow) months = months.slice(months.length - monthsWindow);
...
export function computeTopSkillsTotals(
  ...
  return Object.entries(totals).sort((a, b) => b[1] - a[1]).slice(0, topSkillsCount).map(([skill, count]) => ({ skill, count }));
}
...
export function computeCitySkillTrend(
  ...
  for (const j of jobs) {
    ...
    const cityKey = cities && cities.length > 0 ? (cities.find((c) => normCity(cityRaw).includes(normCity(c))) || null) : cityRaw;
    if (!cityKey) continue;
  ...
  if (!cities || cities.length === 0) {
    topCities = Object.entries(totalsByCity).sort((a, b) => b[1] - a[1]).slice(0, topCityCount).map(([city]) => city);
    seriesCities = topCities;
  } else {
    topCities = Object.entries(totalsByCity).sort((a, b) => b[1] - a[1]).map(([city]) => city);
  }
...
export function computeMetrics(
  ...
  const topSkills = Object.entries(skillTotals).sort((a, b) => b[1] - a[1]).slice(0, topSkillsCount).map(([s]) => s);
  const seriesSkills = (seriesOverride && seriesOverride.length > 0) ? Array.from(new Set(seriesOverride)) : topSkills;
