...
export default function Charts({ metrics, months = 12, smooth = false, mode = 'basics' }: Props) {
  ...
  function SimpleTooltip({ active, payload, label, unit }: { active?: boolean; payload?: any[]; label?: string; unit?: string }) {
    if (!active || !payload || payload.length === 0) return null;
    const val = payload[0]?.value as number | undefined;
    return (
      <Box bg="white" borderWidth="1px" rounded="md" p="sm" fontSize="sm" shadow="sm">
        <Text fontWeight="semibold" mb="xs">{label ? fmtMonth(String(label)) : ''}</Text>
        <Text>{typeof val === 'number' ? val.toLocaleString('fr-FR') : '—'}{unit ? ` ${unit}` : ''}</Text>
      </Box>
    );
  }
      ...
      <Box bg="white" borderWidth="1px" rounded="md" p="sm" fontSize="sm" shadow="sm">
        ...
        {payload.map((entry, idx) => (
          <Box key={idx} display="flex" alignItems="center" justifyContent="space-between" gap="md">
            ...
            <Text fontWeight="medium">{typeof entry.value === 'number' ? entry.value.toLocaleString('fr-FR') : '—'}</Text>
          </Box>
        ))}
      </Box>
    );
        ...
        {payload.map((p: any, idx: number) => {
          ...
          return (
            <Box
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); toggleSeries(name); } }}
              role="switch"
  ...
  function ma3(series: Array<Record<string, number | string>>): Array<Record<string, number | string>> {
    ...
    for (let i = 0; i < series.length; i++) {
      ...
      Object.keys(acc).forEach((k) => { (out[i] as any)[k] = Math.round((acc[k] / count) * 100) / 100; });
    }
    return out;
  }
  ...
  const skillsPerMonth = ma3(metrics.skillsPerMonth.filter((p) => keptMonths.includes(String(p.month))));

  // Allow toggling series visibility for skills
  const [hidden, setHidden] = React.useState<string[]>([]);
  function toggleSeries(name: string) { setHidden((prev) => prev.includes(name) ? prev.filter((n) => n !== name) : [...prev, name]); }
        ...
        <Box borderWidth="0px" p={0} bg="transparent" shadow="none">
          ...
          <Box h="16rem">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={postingsPerMonth} margin={{ top: 10, right: 20, bottom: 0, left: 0 }}>
  ...
  return (
    <Box>
      ...
      <Text fontSize="xs" color="gray.600" mb="sm">Ajoutez/retirez des skills via le sélecteur, et cliquez sur la légende pour masquer/afficher une série.</Text>
      <Box h="20rem">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={skillsPerMonth} margin={{ top: 10, right: 20, bottom: 0, left: 0 }}>
            ...
            {metrics.seriesSkills.filter((s) => !hidden.includes(s)).map((s, idx) => (
