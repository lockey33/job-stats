...
export default function EmergingSkillsChart({ payload, limit = 10 }: Props) {
  ...
  const trends = [...payload.trends].sort((a, b) => (b.slope ?? 0) - (a.slope ?? 0)).slice(0, Math.max(1, limit));
    ...
    return (
      <Box display="flex" flexWrap="wrap" gap="sm" mt="xs">
        {payload.map((p: any, idx: number) => (
          <Box key={idx} display="inline-flex" alignItems="center" gap="xs" px="xs" py="1" rounded="md" borderWidth="1px">
