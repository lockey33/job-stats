...
async function readMergedFromChunks(): Promise<{ jobs: JobItem[]; version: string }> {
  ...
  const files = names
    ...
    .sort((a, b) => a.n - b.n);
  if (files.length === 0) throw new Error('No numeric data files found in src/data for fallback build');
  ...
  for (const f of files) {
    ...
    try { parsed = JSON.parse(raw); }
      ...
      const sanitized = raw.replace(/[\u0000-\u001F]/g, ' ');
      try { parsed = JSON.parse(sanitized); } catch { parsed = {}; }
    }
    const arr: unknown[] = Array.isArray(parsed) ? parsed : (Array.isArray(parsed?.data) ? parsed.data : []);
    for (const it of arr) {
      const res = JobItemSchema.safeParse(it);
